...
// Global variables
let scene, camera, renderer, controls, mouthModel;
let isLoading = true;
...
function init() {
    ...
    // Camera setup - Professional settings for mouth viewing
    camera = new THREE.PerspectiveCamera(
        75, // FOV - wider for detailed mouth viewing
        window.innerWidth / window.innerHeight,
        0.1, // Near plane
        1000 // Far plane
    );
    
    // Position camera for optimal mouth viewing
    camera.position.set(0, 0, 8);
    camera.lookAt(0, 0, 0);
    
    // Renderer setup with high quality settings
    renderer = new THREE.WebGLRenderer({
        canvas: document.getElementById('mouthCanvas'),
        antialias: true,
        alpha: true,
...
function setupControls() {
    ...
    // Zoom settings - responsive range for mouth viewing
...
function loadMouthModel() {
    const loader = new GLTFLoader();
    
    loader.load('human_mouth_detailed (1).glb', function(gltf) {
        mouthModel = gltf.scene;
        
        // Scale the model appropriately for mouth viewing
        mouthModel.scale.setScalar(3.0);
        
        // Center the model
        const box = new THREE.Box3().setFromObject(mouthModel);
        const center = box.getCenter(new THREE.Vector3());
        mouthModel.position.sub(center);
        ...
        // Setup mouth part highlighting
        setupMouthPartHighlighting();
        ...
        document.getElementById('mouth-parts').style.display = 'flex';
        isLoading = false;
        
        console.log('✅ Human mouth model loaded successfully!');
        
    }, function(progress) {
        const percent = Math.round((progress.loaded / progress.total) * 100);
        document.getElementById('loading').textContent = `Loading 3D Human Mouth... ${percent}%`;
    }, function(error) {
        console.error('❌ Error loading mouth model:', error);
        document.getElementById('loading').textContent = 'Error loading mouth model. Please check console.';
    });
}
...
function preserveOriginalTextures() {
    ...
    // Preserve original GLB textures and materials
    mouthModel.traverse(function(child) {
        if (child.isMesh) {
...
function setupMouthPartHighlighting() {
    // Setup mouth part buttons
    const mouthPartButtons = document.querySelectorAll('.mouth-part-btn');
    
    mouthPartButtons.forEach(button => {
        button.addEventListener('click', function() {
            const partName = this.getAttribute('data-part');
            highlightMouthPart(partName);
            
            // Update button states
            mouthPartButtons.forEach(btn => btn.classList.remove('active'));
...
function highlightMouthPart(partName) {
    ...
    // Define mouth part highlighting logic
...
function shouldHighlightMesh(meshName, partName) {
    // Simple mapping logic for mouth anatomy
    const mappings = {
        'teeth': ['tooth', 'teeth', 'incisor', 'canine', 'molar', 'premolar'],
        'tongue': ['tongue', 'lingual'],
        'gums': ['gum', 'gingiva', 'gingival'],
        'palate': ['palate', 'palatal', 'hard', 'soft'],
        'uvula': ['uvula', 'uvular'],
        'lips': ['lip', 'labial', 'labium']
    };
...
function resetHighlights() {
    ...
    // Reset button states
    document.querySelectorAll('.mouth-part-btn').forEach(btn => {
