...
const TABLE_NAME = 'downloadAnswer';
...
const getDownloadAnswers = async (queryStringParameters) => {
  let connection;
  try {
    ...
    const year = params.year != null ? cleanParam(params.year) : null;

    let query = `SELECT id, name, email, phone, city, year, created_at FROM ${TABLE_NAME} WHERE 1=1`;
    const queryParams = [];
    ...
    console.error('Error in getDownloadAnswers:', error);
...
const createDownloadAnswer = async (body) => {
  let connection;
  try {
    ...
    if (!name || !email || !phoneRaw || !city || !year) {
      return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: 'name, email, phone, city, and year are required' }) };
    }
    ...
    const [result] = await connection.execute(
      `INSERT INTO ${TABLE_NAME} (name, email, phone, city, year) VALUES (?, ?, ?, ?, ?)`,
      [name, email, phone, city, year],
    );

    return { statusCode: 201, headers: corsHeaders, body: JSON.stringify({ message: 'Download answer created successfully', id: result.insertId }) };
  } catch (error) {
    console.error('Error in createDownloadAnswer:', error);
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ message: 'Internal Server Error', error: error.message }) };
  } finally {
    if (connection) {
      try { connection.release(); } catch (_) {}
...
const updateDownloadAnswer = async (body) => {
  let connection;
  try {
    const data = typeof body === 'string' ? JSON.parse(body) : body;
    if (!data || typeof data !== 'object') {
    ...
    if (data.year !== undefined) { updateFields.push('year = ?'); updateParams.push((data.year == null ? null : String(data.year).trim()) || null); }
    ...
    return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: 'Download answer updated successfully' }) };
  } catch (error) {
    console.error('Error in updateDownloadAnswer:', error);
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ message: 'Internal Server Error', error: error.message }) };
  } finally {
    if (connection) {
      try { connection.release(); } catch (_) {}
...
const deleteDownloadAnswer = async (body) => {
  let connection;
  try {
    const data = typeof body === 'string' ? JSON.parse(body) : body;
    if (!data || typeof data !== 'object') {
    ...
    console.error('Error in deleteDownloadAnswer:', error);
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ message: 'Internal Server Error', error: error.message }) };
  } finally {
    if (connection) {
      try { connection.release(); } catch (_) {}
...
export const handler = async (event) => {
  ...
  try {
    switch (httpMethod) {
      case 'GET':
        return await getDownloadAnswers(queryStringParameters);
      case 'POST':
        return await createDownloadAnswer(parsedBody || body);
      case 'PUT':
        return await updateDownloadAnswer(parsedBody || body);
      case 'DELETE':
        return await deleteDownloadAnswer(parsedBody || body);
      default:
